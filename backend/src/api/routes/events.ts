...
router.get("/", (req: Request, res: Response) => {
  ...
  const { data, total } = eventStore.query(query);
...
router.get("/recent", (_req: Request, res: Response) => {
  const data = eventStore.getAll(200);
  res.json({ success: true, data, count: data.length });
});

// GET /api/events/by-source/:source
router.get("/by-source/:source", (req: Request, res: Response) => {
  const source = String(req.params.source);
  const limit = parseInt((req.query.limit as string) ?? "100", 10);
  const data = eventStore.getBySource(source, limit);
  res.json({ success: true, data, count: data.length, source });
});

// GET /api/events/stats
router.get("/stats", (_req: Request, res: Response) => {
  const all = eventStore.getAll(eventStore.count());
